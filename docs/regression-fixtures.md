...
| Radio recovery | `unavailable_radio_rejects_queued_ptt_without_touching_driver` | A queued acknowledged PTT command is rejected with an explicit powered-off error while the worker remains in a safe state. | Physical disconnect timing and reconnect behavior on a real transport. |
