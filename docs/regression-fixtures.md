...
| ADIF | `imports_adif_records_and_skips_duplicates`, `imports_contest_pota_grid_and_malformed_fields_without_losing_valid_records`, `adif_round_trip_preserves_qsonaut_metadata` and related tests | Import normalization, duplicate handling, contest/POTA/grid fields, QSONaut metadata round trips, malformed records, and export fields are stable for covered records. | Interoperability with every external logger. |
