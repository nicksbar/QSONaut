...
async fn main() -> Result<()> {
    ...
    if has_radio_ops {
        ...
        if cli.enable_spectrum_stream && cli.disable_spectrum_stream {
            anyhow::bail!("--enable-spectrum-stream and --disable-spectrum-stream are mutually exclusive");
        }
        ...
        if let Some(control) = cli.set_control {
            ...
            let raw_value = cli
                .control_value
                .as_deref()
                .with_context(|| format!("--control-value is required for --set-control {control:?}"))?;
    ...
    if cli.gui || launch_gui_by_default {
        ...
        let gui_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| run_gui(config.clone())));
        match gui_result {
            Ok(Ok(())) => {}
            Ok(Err(err)) => {
...
fn parse_control_value(id: ControlId, raw: &str) -> Result<ControlValue> {
    match id {
        ...
        | ControlId::Agc
        | ControlId::Filter => {
            let v: u8 = raw.parse().with_context(|| format!("invalid numeric control value: {raw}"))?;
