...
pub fn codeword_to_tones(codeword: &[u8; N]) -> [u8; N_SYM] {
    ...
    for sym in 0..N_SYM {
        ...
        if is_sync {
            let costas_idx = if sym < 7 { sym } else if sym < 43 { sym - 36 } else { sym - 72 };
            tones[sym] = COSTAS[costas_idx];
        } else {
            let b = data_idx * 3;
            let idx = (channel[b] as usize) << 2 | (channel[b + 1] as usize) << 1 | channel[b + 2] as usize;
...
mod tests {
    ...
    fn tone_count_and_sync_positions() {
        let msg = Ft8Message::Standard {
            call1: "CQ".into(), call2: "N0CALL".into(),
            report: "EN40".into(), ir: false,
        };
        if let Some(tones) = message_to_tones(&msg) {
            ...
            for (i, &t) in tones.iter().enumerate() {
                if i < 7 || (i >= 36 && i < 43) || i >= 72 {
                    let ci = if i < 7 { i } else if i < 43 { i - 36 } else { i - 72 };
