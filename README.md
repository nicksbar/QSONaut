...
| SSTV and local images | Auto VIS receive and manual receive filtering decode 13 Martin/Scottie/Robot/PD modes with movable audio-window alignment. The pinned `komitoto-sstv` adapter also provides selectable experimental TX for those modes. Existing images can be browsed or generated through local Ollama/Lemonade models, and TX remains explicitly armed. |
